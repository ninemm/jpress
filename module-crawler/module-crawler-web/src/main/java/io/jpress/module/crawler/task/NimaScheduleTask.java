...
import io.jpress.module.crawler.crawler.ProxyKuaiCrawler;
import io.jpress.module.crawler.crawler.ProxyNiMaCrawler;
 ...
 */
public class NimaScheduleTask extends AbstractScheduleTask<ProxyInfo> {

    private static final Log _LOG = Log.getLog(NimaScheduleTask.class);

    public NimaScheduleTask(ScheduleTask scheduleTask) {
        super(scheduleTask);
    }
    ...
    public void run() {
        ...
        AbstractBreadthCrawler crawler = new ProxyNiMaCrawler("crawler/nima", false, spider);
        try {
            ProxyCrawlerManager.me().start(ProxySite.nima.getKey(), crawler, 1);
        } catch (Exception e) {
