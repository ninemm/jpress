...
public class SogoRelWordsCrawler extends AbstractRamCrawler {

    private static final Log _LOG = Log.getLog(SogoRelWordsCrawler.class);

    public SogoRelWordsCrawler(KeywordParamVO keyword, String searchType, Integer pageIndex) {
    ...
    public void visit(Page page, CrawlDatums next) {
        ...
        Elements results = page.select("#hint_container>tbody>tr>td>p>a");
        _LOG.info("关键词：" + keyword + ", 在搜狗中的相关(搜索)关键词个数：" + results.size());
    ...
    public String createUrl(String keyword, Integer pageIndex) {
        try {
            ...
            return String.format("http://www.sogo.com/web?query=%s", keyword);
